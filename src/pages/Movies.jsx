...
import { fetchTopFilms } from "../Components/fetchAPI";
...
const Movies = () => {
  ...
  const onClick = () => {
    ...
    const fetch = async () => {
      ...
      const res = await fetchTopFilms(pageRef.current);
      console.log(res);
  ...
  useEffect(() => {
    const fetchTrend = async () => {
      try {
        const resp = await fetchTopFilms(pageRef.current);
        console.log(resp);
        setResults(resp);
      } catch (error) {
        setError(true);
  ...
  return (
    <div className="w-full h-[100vh] bg-black">
      <motion.div className="w-full h-fit bg-black pt-[136px] pb-10">
        {modalData.openMovie &&
          createPortal(
            <BackdropModal>
