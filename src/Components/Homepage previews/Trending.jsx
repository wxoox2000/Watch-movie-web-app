...
if(modalRef.current.getBoundingClientRect().top < 0){
  ...
  const TrendingNavRef = useRef(0);
  let count = 0;
  return (
    <div  ref={modalRef} className="w-[1092px] mb-8">
      <Swiper
        className="pr-[10px]"
        onProgress={(p) => (TrendingNavRef.current = p.progress)}
        spaceBetween={24}
        slidesPerView={4}
      >
        ...
        <SwiperSlide>
          <Link to="trending" className="w-64 h-[301px] block shrink-0 hover:shadow-xlInn px-5 py-8 rounded-2xl border border-gray hover:shadow-shadowColor trans">
        ...
        <SlideNextButton ref={TrendingNavRef} />
        <SlidePrevButton ref={TrendingNavRef} />
      </Swiper>
